...
3. 在持仓卡片上点击“更新阶段判断”，查看“低位观察、底部确认、中性、高位观察、顶部确认”或“数据不足”。
4. 点击“历史模拟”，比较同一段历史中“全程持有基准”和“核心仓 + 机动仓模拟”的差异。
...
| 布林带位置 | 观察价格是否靠近波动区间边缘，并参与阶段判断 |
| ATR | 记录最近波动有多大，作为复盘背景，不单独触发阶段判断 |
| 相对量能 | 记录成交量相对过去一段时间的变化，作为复盘背景，不单独触发阶段判断 |
| 反转确认 | 结合收盘回稳/转弱和锤子线、十字等辅助形态，避免只凭一根 K 线下结论 |

主证据是 120 日价格位置和回撤，再由 RSI/布林带、MA 环境、收盘转折和辅助形态交叉确认；规则刻意保持简单，不堆叠大量参数。

这些条件是“观察信号”，不是精确抄底或逃顶工具。趋势很强时，价格可能在高位继续上涨；下跌途中也可能多次出现看起来像底部的形态。因此页面不会把阶段标签写成操作指令。

#### 为什么不是高频交易

- 只使用已经收盘的日线，观察节奏是数周到数月，不处理分钟线和盘口变化。
- 历史模拟遵循“`t` 日收盘确认状态，下一根日线开盘模拟变化”，不会偷看当天收盘后的未来价格。
- 至少需要 120 根完整日线；数据不足时只显示“数据不足”，不会编造收益率。
- 默认 20% 只是可编辑的历史模拟假设，允许在持仓卡片中调整；它不代表你的真实配置。模拟中的 80% 核心仓始终保持不变。

#### 股票、ETF 和成本边界

股票和 ETF 的交易制度、申赎方式、是否支持当日回转、最小申报单位及费率可能不同，不能只按名称猜测。工具要求明确选择“股票”或“ETF”，手续费、滑点和可能的单向税费是报告明示的固定示例假设，当前网页不可编辑，实际规则和券商费率需要自行核对。工具不会把成本价当作阶段判断依据，也不会据此自动改变你的真实持仓。

#### 数据和隐私边界

当前使用免费公开日线接口。公开接口可能延迟、限流、缺少交易日或临时不可用；复权方式、缺失数据和接口质量都会影响历史比较。数据源失败或不完整时，报告会明确提示，不用旧数据冒充新结果。

个人持仓文件、最近结果和报告固定放在仓库的 `data/private/` 下，并由 Git 忽略；它们只用于本机网页和本机运行，不应复制到 GitHub。代码仓库和测试只使用虚构数据，不能把真实代码、数量、成本或截图提交到公开仓库。建议你自行备份 `data/private/`，但备份也要放在自己信任的位置。
