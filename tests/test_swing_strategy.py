...
def test_state_machine_has_watch_then_confirmed_bottom_without_order_language(strategy_config) -> None:
    previous = _feature(0, close=90.0, position=0.10, drawdown=-0.30, rsi=25.0, bb=0.10)
    current = _feature(
        1,
        close=91.0,
        position=0.12,
    ...
    confirmed = evaluate_swing_state(current, previous=previous, config=strategy_config)
...
def test_state_machine_has_watch_then_confirmed_top(strategy_config) -> None:
    previous = _feature(0, close=110.0, position=0.90, drawdown=-0.01, rsi=75.0, bb=0.90)
    current = _feature(
        1,
        close=109.0,
        position=0.88,
